...
class Player
{
    ...
    private static void HeroCommands(Util util, Dictionary<string, Hero> enemyHeros)
    {
        ...
        {
            var spellToUse = myHero.Spells.FirstOrDefault(x => x.Type == "Defensive" && myCurrentHero.coolDowns[x.CoolDownSlot] == 0 && myCurrentHero.mana >= x.ManaCost);
        ...
        else
        {
            ...
            if (eHeros.Any())
            {
                var spellsToUse = myHero.Spells.Where(x => x.Type == "Offensive" && myCurrentHero.coolDowns[x.CoolDownSlot] == 0 && myCurrentHero.mana >= x.ManaCost);
                var eHero = eHeros.First();
                var spellUsed = false;
                foreach (var spell in spellsToUse)
                {
                    if (spell.Range > util.Hypotenuse(Math.Abs(eHero.x - myCurrentHero.x), Math.Abs(eHero.y - myCurrentHero.y)))
                    {
                        if (spell.Type == "AOE")
                        {
    ...
    {
        // TODO write find lower ally , enemy etc
    }
}
...
class Util
{
    ...
    public void UpdateWithRoundInfo(List<Bush> bushes, string heroName, List<Entity> entities, int myTeam, List<Item> shop, int gold, Hero hero)
    {
        this.bushes = bushes;
    ...
    {
        return bushes.OrderBy(bush => Hypotenuse(Math.Abs(myCurrentHero.x - bush.x), Math.Abs(myCurrentHero.y - bush.y))).First();
    }
    ...
    {
        return shop.Where(x => x.isPotion == 1 && x.health > 0 && x.itemCost < gold && myCurrentHero.IsLowHealth).ToList();
    }
